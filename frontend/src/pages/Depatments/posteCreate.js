...
function PosteCreate() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    const section = "Poste";
    formData.departmentId = parseInt(id, 10);
    try {
      await postData("/postes", formData, Cookies.get("jwt"));
      navigate("/valide", { state: { section } });
    } catch (error) {
      if (error.response && error.response.data.errors) {
        setCurrentErrorMessage(Object.values(error.response.data.errors)[0]);
      } else {
        navigate("/nonvalide", { state: { section } });
      }
    }
  };
