...
function ComponentEdit() {
  ...
  const handleComposant = async () => {
    ...
    setFormData({
      name: response.name,
      componentImg: response.componentImg,
      description: response.description,
    });
    const imgUrl = await getUserImg(
      `/components/componentImage/${response.componentImg}`,
      Cookies.get("jwt")
    );
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      setFormData({ ...formData, componentImg: imgName.filename });
      const imgUrl = await getUserImg(
        `/components/componentImage/${imgName.filename}`,
        Cookies.get("jwt")
      );
      setUserImgUrl(imgUrl);
    } catch (error) {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const newcomponent = await update(
        `/components/${idComponent}/edit`,
        updatedFormData,
        Cookies.get("jwt")
      );
      navigate("/valide");
    ...
    <div className="table__page__container">
      ...
      <div className="table__page__content__container">
        ...
        <div className="ticketForm__content">
          <header>Editer Composant</header>
          <form>
            ...
            <div className="buttons">
              <button className="nextBtn" type="button" onClick={handleEdit}>
